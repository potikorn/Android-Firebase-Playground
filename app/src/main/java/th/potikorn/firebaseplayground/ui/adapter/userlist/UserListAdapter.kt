...
class UserListAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) =
        (holder as UserListViewHolder).onBindData(items?.get(position), itemActionsListener)
    ...
    fun setOnActionListener(listener: BaseAdapterListener) {
        itemActionsListener = listener
    }
}